"""
Test KP14 Moment Calculations - Regression Test
...
PURPOSE:
--------
This test verifies that the refactored KP14 (Kogan-Papanikolaou 2014) SDF
...
ORIGINAL CODE (tests/original_code/):
  ...
  1. Import sdf_compute_kp14.py (KP14 SDF moment calculation module)
     - Imports: parameters_kp14.py (model parameters including alpha, r, gamma_x, gamma_z)
     - Imports: panel_functions_kp14.py (for accessing panel data and K matrix)
  ...
  2. Function: sdf_compute(N, T, arrays)
     ...
     - Inputs:
       ...
       * arrays: Tuple of 22 arrays from panel generation, including K matrix
     ...
     - Returns: sdf_loop(month, panel_id) function
       ...
       * Returns: (sdf_ret, max_sr, rp, cond_var)
         ...
         - rp: Risk premia (2 x 1 array for x and z factors)
  ...
  3. Computation workflow:
     ...
     - Use K matrix to compute pricing kernel components
       NOTE: K matrix access: K[:t+1, t, :] requires t < K.shape[1]
     - Compute SDF using: m = M_ratio * (consumption ratio)^(-1/psi)
  ...
  4. Execution:
     ...
     - Import sdf_compute_kp14 module
     - Call sdf_compute(N, T, arrays) to create sdf_loop function
     - Call sdf_loop(month, panel_id) for each month

  5. Key difference from BGN: K matrix usage
     - BGN uses simple exponential pricing kernel
     - KP14 uses recursive pricing with K matrix for state-dependent pricing
     - K matrix must be pre-computed or computed on-demand

CURRENT CODE (utils_kp14/):
  Implementation: Run from the terminal via subprocess (../calculate_moments.py)

  1. Import sdf_compute_kp14.py
     - Imports: config.py (centralized configuration)
     - Imports: panel_functions_kp14.py (refactored version)
  ...
  3. Execution:
     - Run calculate_moments.py via subprocess: python calculate_moments.py kp14_0
     - Script reads panel from pickle file (kp14_0_panel.pkl)
     - Computes moments using sdf_compute_kp14
     - Saves moments to pickle file: kp14_0_moments.pkl
     - Test reads pickle file to get results

  4. Potential optimization (if implemented):
     - On-demand K matrix computation instead of full pre-allocation
     - Should produce identical results with lower memory usage
...
Test months selected:
  - Month 660 = KP14_BURNIN + 360 (middle of post-burnin period)
  - Month 700 = T + KP14_BURNIN (last month, but may be out of bounds)
  - Month 650 = T + KP14_BURNIN - 50 (near end)

Note: Month indices passed to sdf_loop are 0-based, so we use (month - 1)
Important: Month index must satisfy: month - 1 < K.shape[1]
...
3. Read panel pickle (kp14_0_panel.pkl) and moments pickle (kp14_0_moments.pkl)
...
Requires:
  ...
  - Original code SDF computation (tests/original_code/sdf_compute_kp14.py)
...
def test_kp14_moment_calculations():
    """Test KP14 moment calculations by generating panel, testing moments, then cleanup."""
    print("=" * 70)
    ...
    cmd = [sys.executable, str(parent_dir / 'calculate_moments.py'), f'kp14_{TEST_PANEL_ID}']
    ...
    panel_file = Path(DATA_DIR) / f'kp14_{TEST_PANEL_ID}_panel.pkl'
    moments_file = Path(DATA_DIR) / f'kp14_{TEST_PANEL_ID}_moments.pkl'
    ...
    sdf_loop_old = sdf_old.sdf_compute(N, T + KP14_BURNIN, arrays)

    # Test a few months
    # Note: These are absolute month indices including burnin
    # Test first month, last month, and a random middle month
    middle_month = randint(KP14_BURNIN + 361, KP14_BURNIN + T - 2)
    test_months = [KP14_BURNIN + 360, KP14_BURNIN + T - 1, middle_month]
    ...
    print(f"\n{'-'*70}")
    if tests_run == 0:
        print(f"  [FAIL] No tests were run! Expected to test {len([m for m in test_months if m < T + KP14_BURNIN])} months")
        all_passed = False
    elif all_passed:
    ...
    csv_file = Path(__file__).parent / 'port_kp.csv'
