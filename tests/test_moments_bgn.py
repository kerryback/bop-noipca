"""
Test BGN Moment Calculations - Regression Test
...
PURPOSE:
--------
This test verifies that the refactored BGN SDF (Stochastic Discount Factor)
...
ORIGINAL CODE (tests/original_code/):
  ...
  1. Import sdf_compute.py (BGN SDF moment calculation module)
     - Imports: parameters.py (model parameters: r, gamma_x, gamma_z, etc.)
     - Imports: panel_functions.py (for accessing panel data)
  ...
  2. Function: sdf_compute(N, T, arrays)
     ...
     - Inputs:
       ...
       * arrays: Tuple of 17 arrays from panel generation
     ...
     - Returns: sdf_loop(month, panel_id) function
       ...
       * Returns: (sdf_ret, max_sr, rp, cond_var)
         ...
         - rp: Risk premia (2 x 1 array for mu and xi factors)
  ...
  3. Computation workflow:
     ...
     - Compute SDF using pricing kernel: m = exp(-r * dt - gamma_x * f_mu - gamma_z * f_xi)
...
CURRENT CODE (utils_bgn/):
  ...
  1. Import sdf_compute_bgn.py
     - Imports: config.py (centralized configuration)
     - Imports: panel_functions_bgn.py (refactored version)
  ...
  3. Execution:
     - Run calculate_moments.py via subprocess: python calculate_moments.py bgn_0
     - Script reads panel from pickle file (bgn_0_panel.pkl)
     - Computes moments using sdf_compute_bgn
     - Saves moments to pickle file: bgn_0_moments.pkl
...
Test months selected:
  - Month 660 = BGN_BURNIN + 360 (middle of post-burnin period)
  - Month 700 = T + BGN_BURNIN (last month, but may be out of bounds)
  - Month 650 = T + BGN_BURNIN - 50 (near end)
...
3. Read panel pickle (bgn_0_panel.pkl) and moments pickle (bgn_0_moments.pkl)
...
Requires:
  ...
  - Original code SDF computation (tests/original_code/sdf_compute.py)
  - Working directory changed to tests/original_code/ for data file access
"""
...
def test_bgn_moment_calculations():
    """Test BGN moment calculations by generating panel, testing moments, then cleanup."""
    print("=" * 70)
    ...
    panel_file = Path(DATA_DIR) / f'bgn_{TEST_PANEL_ID}_panel.pkl'
    moments_file = Path(DATA_DIR) / f'bgn_{TEST_PANEL_ID}_moments.pkl'
    ...
    sdf_loop_old = sdf_old.sdf_compute(N, T + BGN_BURNIN, arrays)

    # Test a few months
    # Note: These are absolute month indices including burnin
    # Test first month, last month, and a random middle month
    middle_month = randint(BGN_BURNIN + 361, BGN_BURNIN + T - 2)
    test_months = [BGN_BURNIN + 360, BGN_BURNIN + T - 1, middle_month]
    ...
    print(f"\n{'-'*70}")
    if tests_run == 0:
        print(f"  [FAIL] No tests were run! Expected to test {len([m for m in test_months if m < T + BGN_BURNIN])} months")
        all_passed = False
    elif all_passed:
    ...
    print(f"\n[SAVE] Saving moments to tests/outputs...")
    import shutil
    outputs_dir = Path(__file__).parent / 'outputs'
    outputs_dir.mkdir(exist_ok=True)

    # Save moments file from new code
    if moments_file.exists():
        moments_save = outputs_dir / f'bgn_{TEST_PANEL_ID}_moments_new.pkl'
        shutil.copy2(moments_file, moments_save)
        print(f"      Saved new moments to {moments_save.name}")

    # Save moments from original code
    print(f"      Computing original moments for all months...")
    moments_original = {}
    for month in range(start_month, end_month + 1):
        sdf_ret_old, max_sr_old, rp_old, cond_var_old = sdf_loop_old(month - 1, 0)
        moments_original[month] = {
            'sdf_ret': sdf_ret_old,
            'max_sr': max_sr_old,
            'rp': rp_old,
            'cond_var': cond_var_old
        }

    original_moments_data = {
        'moments': moments_original,
        'N': moments_data['N'],
        'start_month': start_month,
        'end_month': end_month,
        'panel_id': f'bgn_{TEST_PANEL_ID}'
    }

    moments_original_file = outputs_dir / f'bgn_{TEST_PANEL_ID}_moments_original.pkl'
    with open(moments_original_file, 'wb') as f:
        pickle.dump(original_moments_data, f)
    print(f"      Saved original moments to {moments_original_file.name}")

    # Cleanup: delete files from DATA_DIR and tests/
    csv_file = Path(__file__).parent / 'port_bgn.csv'
