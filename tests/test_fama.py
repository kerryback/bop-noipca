...
def test_fama_factors(model):
        ...
        if not moments_file.exists():
            ...
            for test_month in test_months:
                ...
                try:
                    ...
                    assert_close(expected_sdf_ret, fama_row['sdf_ret'], rtol=1e-14, atol=1e-15, name="sdf_ret")
    ...
    print(f"\n[SAVE] Saving Fama results to tests/outputs...")
    import shutil
    ...
    if fama_file.exists():
        dst_file = outputs_dir / f'{panel_id}_fama_new.pkl'
        shutil.copy2(fama_file, dst_file)
        print(f"      Saved new Fama to {dst_file.name}")

    # Save original code results
    fama_original_data = {
        'ff_returns': ff_returns_old,
        'fm_returns': fm_returns_old
    }
    fama_original_file = outputs_dir / f'{panel_id}_fama_original.pkl'
    with open(fama_original_file, 'wb') as f:
        pickle.dump(fama_original_data, f)
    print(f"      Saved original Fama to {fama_original_file.name}")

    # Cleanup - delete all pickle files from DATA_DIR
    print(f"\n[CLEANUP] Deleting pickle files from DATA_DIR...")

    files_to_delete = [fama_file]
